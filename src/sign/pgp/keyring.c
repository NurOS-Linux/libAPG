...
keyring_load(const char *keyring_dir)
{
    ...
    if (keyring_dir)
        gpgme_ctx_set_engine_info(kr->ctx, GPGME_PROTOCOL_OpenPGP,
                                  NULL, keyring_dir);
    return kr;
}
...
keyring_verify(const struct keyring *kr, const char *pkg_path,
               const char *sig_path)
{
    if (!kr || !pkg_path || !sig_path) return false;
    ...
    if (!result || !result->signatures) return false;
                ...
                const char *key_sig_path, const struct keyring *trusted)
{
    ...
    (void)keyring_dir;
    (void)key_sig_path;
    if (!trusted || !new_key_path) return false;

    gpgme_data_t key_data;
    if (gpgme_data_new_from_file(&key_data, new_key_path, 1) != GPG_ERR_NO_ERROR)
        return false;
    ...
    if (!import_result || import_result->imported == 0) return false;


    gpgme_import_status_t status = import_result->imports;
    while (status) {
        gpgme_key_t key;
        if (gpgme_get_key(trusted->ctx, status->fpr, &key, 0) == GPG_ERR_NO_ERROR) {
            gpgme_validity_t v = key->uids ? key->uids->validity
