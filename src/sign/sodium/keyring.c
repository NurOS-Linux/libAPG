...
keyring_load(const char *keyring_dir)
{
    ...
    if (!dir) { free(kr); return NULL; }

    struct dirent *entry;
    while ((entry = readdir(dir)) != NULL) {
        if (!has_key_ext(entry->d_name)) continue;
...
keyring_verify(const struct keyring *kr, const char *pkg_path,
               const char *sig_path)
{
    if (!kr || !pkg_path || !sig_path || kr->count == 0) return false;
...
{
    if (!keyring_dir || !new_key_path || !key_sig_path || !trusted) return false;
