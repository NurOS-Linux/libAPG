...
pack_key(uint8_t out[12], time_t ts, uint32_t seq)
{
    ...
    out[1] = t & 0xff; t >>= 8;
    out[0] = t & 0xff;
    out[11] = seq & 0xff; seq >>= 8;
    out[10] = seq & 0xff; seq >>= 8;
    out[9]  = seq & 0xff; seq >>= 8;
    out[8]  = seq & 0xff;
}
...
journal_read_all(MDB_env *env, int *count)
{
    *count = 0;
    ...
    while (mdb_cursor_get(cursor, &k, &v, MDB_NEXT) == MDB_SUCCESS) {
        ...
        if (fi < 3) { free(raw); continue; }

        struct journal_entry *e = malloc(sizeof(*e));
        if (!e) { free(raw); continue; }

        e->op        = strcmp(fields[0], "install") == 0 ? JOURNAL_INSTALL : JOURNAL_REMOVE;
        e->pkg_name  = strcmp(fields[1], "-") == 0 ? NULL : strdup(fields[1]);
        e->pkg_version = strcmp(fields[2], "-") == 0 ? NULL : strdup(fields[2]);
        e->status    = strcmp(fields[3], "ok") == 0 ? JOURNAL_STATUS_OK : JOURNAL_STATUS_FAILED;
        ...
        if (*count == cap) {
            ...
            if (!tmp) { journal_entry_free(e); break; }
            list = tmp;
        }
        list[(*count)++] = e;
    }
