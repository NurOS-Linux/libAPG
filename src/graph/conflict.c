...
dep_error_t
dep_graph_find_breaks(struct dep_graph *g,
                      const char *pkg_name,
                      ...
                      size_t *break_count)
{
    ...
    for (size_t i = 0; i < installed_count; i++) {
        ...
        if (!conflict && inst_idx != SIZE_MAX) {
            ...
            // Installed package conflicts with something the new package provides
            if (!conflict) {
                for (int j = 0; j < pkg->provides.count && !conflict; j++)
                    if (str_list_contains(&inst->conflicts, pkg->provides.items[j]))
                        conflict = true;
            }

            // New package conflicts with something the installed package provides
            if (!conflict) {
                for (int j = 0; j < inst->provides.count && !conflict; j++)
                    if (str_list_contains(&pkg->conflicts, inst->provides.items[j]))
                        conflict = true;
            }
        }
