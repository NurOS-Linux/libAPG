...
parse_str_array(yyjson_val *arr)
{
    ...
    {
        list.items[idx] =
            yyjson_is_str(val) ? strdup(yyjson_get_str(val)) : NULL;
    }
...
static void
add_str_array(yyjson_mut_doc *doc, yyjson_mut_val *obj, const char *key,
...
parse_metadata_from_root(yyjson_val *root)
{
    ...
    if (yyjson_is_str(v))
        meta->name = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "version");
    if (yyjson_is_str(v))
        meta->version = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "type");
    if (yyjson_is_str(v))
        meta->type = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "architecture");
    if (yyjson_is_str(v))
        meta->architecture = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "description");
    if (yyjson_is_str(v))
        meta->description = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "maintainer");
    if (yyjson_is_str(v))
        meta->maintainer = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "license");
    if (yyjson_is_str(v))
        meta->license = strdup(yyjson_get_str(v));

    v = yyjson_obj_get(root, "homepage");
    if (yyjson_is_str(v))
        meta->homepage = strdup(yyjson_get_str(v));
...
package_from_json(const char *json, size_t len)
{
    ...
    yyjson_val *held = yyjson_obj_get(root, "held");
...
package_metadata_from_file(const char *path)
{
    ...
    struct package_metadata *meta =
        yyjson_is_obj(root) ? parse_metadata_from_root(root) : NULL;
...
package_to_json(struct package *pkg)
{
    ...
    yyjson_mut_obj_add_str(doc, root, "architecture",
                           m->architecture ? m->architecture : "");
    yyjson_mut_obj_add_str(doc, root, "description",
                           m->description ? m->description : "");
    yyjson_mut_obj_add_str(doc, root, "maintainer",
