...
dep_constraint_parse(const char *str)
{
    ...
    static const struct { const char *sym; ver_op_t op; } ops[] = {
        { ">=", VER_OP_GE },
        { "<=", VER_OP_LE },
        { "==", VER_OP_EQ },
        { "!=", VER_OP_NEQ },
...
dep_constraint_to_str(const struct dep_constraint *c)
{
    ...
    if (c->op == VER_OP_ANY || !c->version) return strdup(c->name);
    ...
    char *buf = malloc(len);
    if (buf) snprintf(buf, len, "%s %s %s", c->name, op_str, c->version);
    return buf;
}
...
ver_compare(const char *a, const char *b)
{
    ...
    while (*a || *b) {
        ...
        if (ae == a) va = 0; else a = ae;
        if (be == b) vb = 0; else b = be;
...
bool
ver_satisfies(const char *pkg_version, ver_op_t op, const char *constraint_version)
{
    if (op == VER_OP_ANY) return true;
    if (!pkg_version || !constraint_version) return false;

    int cmp = ver_compare(pkg_version, constraint_version);
    switch (op) {
    case VER_OP_EQ:  return cmp == 0;
    case VER_OP_NEQ: return cmp != 0;
    case VER_OP_LT:  return cmp < 0;
    case VER_OP_LE:  return cmp <= 0;
    case VER_OP_GT:  return cmp > 0;
    case VER_OP_GE:  return cmp >= 0;
    default:         return true;
    }
}
