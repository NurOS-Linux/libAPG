...
- `abi-check` CI job (`.github/workflows/ci.yml`, `.forgejo/workflows/ci.yml`): builds the current commit and the previous release tag side by side, dumps both with `abidw`, and fails only on an incompatible ABI change (`abidiff`'s exit status bit 8) against the previous release — purely additive changes (new symbols) pass. `abi-suppressions.txt` excludes the already-opaque handle and read-only result types (`docs/abi-policy.rst`) from the comparison, since `abidw` reads full DWARF debug info and would otherwise flag their private-field changes as false-positive breaks. This closes the last piece of the pre-2.0 ABI-stabilization work tracked in `ROADMAP.md`: ABI regressions were previously only caught by a manual `nm -D` audit
