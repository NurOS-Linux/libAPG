...
make_pkg(const char *name, const char **deps, int dep_count,
         const char **conflicts, int conflict_count, const char **provides,
         int provides_count, const char **replaces, int replaces_count)
{
    ...
    if (dep_count > 0)
    {
        pkg->dependencies.items =
            malloc(dep_count * sizeof(struct dep_constraint));
